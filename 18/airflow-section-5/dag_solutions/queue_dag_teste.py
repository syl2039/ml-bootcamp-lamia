...
# Informações gerais
# - Só troquei as filas das tarefas

# Resultados
# As tarefas 1, 2, 3, 4 e 5 rodarm, mas com um desempenho menor,
# a tarefa 6 não funcionou porque ela requer um ambiente spark configurado
...
with DAG(dag_id='queue_dag', schedule_interval='0 0 * * *', default_args=default_args, catchup=False) as dag:
    ...
    t_2_ssd = BashOperator(task_id='t_2_ssd', bash_command='echo "I/O intensive task"', queue='worker_cpu')
    ...
    t_5_cpu = BashOperator(task_id='t_5_cpu', bash_command='echo "CPU instensive task"', queue='worker_ssd')

    # Tarefa que precisa de workers com as dependencias do spark
    t_6_spark = BashOperator(task_id='t_6_spark', bash_command='echo "Spark dependency task"', queue='worker_ssd')
